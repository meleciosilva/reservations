...
function ReservationForm({ handleNewReservation, handleUpdateReservation }) {
  ...
  function handleErrors() {
    ...
    if (date && currentTime > Date.parse(`${date} ${time}`)) {
      errors.push({
        message:
          "You cannot make a reservation in the past. Select a future date.",
      });
    }
      ...
      <form className="row g-3" onSubmit={isEdit ? handleEdit : handleCreate}>
        ...
        <div className="col-md-6">
          ...
          <input
            ...
            title="mobile number must have at least 7 digits"
        ...
        <div className="col-md-6">
          ...
          <input
            ...
            className="form-control"
            id="reservation_date"
            aria-describedby="reservation_date_help"
            required
            value={state.reservation_date}
            onChange={handleChange}
          />
          <div id="reservation_date_help" className="form-text">
            Closed on Tuesday
          </div>
        </div>
        <div className="col-md-6">
          ...
          <input
            ...
            className="form-control"
            id="reservation_time"
            aria-describedby="reservation_time_help"
            value={state.reservation_time}
            onChange={handleChange}
          />
          <div id="reservation_time_help" className="form-text">
            Hours 10:30am - 10:30pm | Last reservation at 9:30pm
          </div>
        </div>
        <div
          className="btn-group d-flex pb-4"
          role="group"
          aria-label="buttons"
        >
          <button
